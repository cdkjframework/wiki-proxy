...
public class ClientHeartThread implements IClientHeartThread, Runnable {
   ...
   * 日志
   */
  private final LogUtils log = LogUtils.getLogger(ClientHeartThread.class);
  ...
  public void run() {
    ...
    try {
      ...
      log.warn(String.format("{} 心跳异常", clientControlThread.getListenServerPort()));
    ...
    try {
      ...
      log.warn(String.format("重新建立连接" + clientControlThread.getListenServerPort() + "失败第 " + this.failCount + " 次",
          reClientException));
      logFlag = false;
    }
